...
# os.system('wget -O ./brain_region_masks.hdf5 https://huggingface.co/datasets/pscotti/mindeyev2/resolve/main/brain_region_masks.hdf5')
# os.system('wget -O datasets/gnet_multisubject.pt https://huggingface.co/datasets/pscotti/mindeyev2/resolve/main/gnet_multisubject.pt')
...
parser.add_argument(
    "--blend_blurry", action="store_true",
    help="Applica recon*.75 + blurry*.25 per recuperare dettaglio low-level",
)
parser.add_argument(
    "--predcaptions_path", type=str, default=None,
    help="Path alle caption usate per il conditioning. Per ControlNet passa il file delle caption BLIP-2.",
)
if utils.is_interactive():
...
# recon_filename = os.path.basename(all_recons_path)
# model_name_plus_suffix = recon_filename.replace(".pt", "")
# Caption usate per il conditioning del metodo in valutazione.
# Default: GIT (all_predcaptions.pt). Per ControlNet passa
# --predcaptions_path=...all_blip2captions.pt, perche' quel metodo
# condiziona la generazione su BLIP-2, non su GIT.
# if predcaptions_path is None:
#     predcaptions_path = f"evals/{model_name}/{model_name}_all_predcaptions.pt"
# print("predcaptions_path:", predcaptions_path)
# all_predcaptions = torch.load(predcaptions_path)

# # Tag breve per etichettare grafici/print (es. "predcaptions" o "blip2captions")
# caption_source_tag = os.path.basename(predcaptions_path).replace(".pt", "").split("_all_")[-1]
if predcaptions_path is None:
    predcaptions_path = f"evals/{model_name}/{model_name}_all_predcaptions.pt"
print("predcaptions_path:", predcaptions_path)
all_predcaptions = torch.load(predcaptions_path)

# Normalizza il formato: le caption BLIP-2 sono salvate come lista Python,
# quelle GIT come array numpy. Il resto del notebook usa fancy indexing
# (es. all_predcaptions[[j]]), che richiede un array, non una lista.
if isinstance(all_predcaptions, list):
    all_predcaptions = np.array(all_predcaptions, dtype=object)

caption_source_tag = os.path.basename(predcaptions_path).replace(".pt", "").split("_all_")[-1]

# model name — derivato dal file effettivamente caricato, non hardcoded
recon_filename = os.path.basename(all_recons_path)
model_name_plus_suffix = recon_filename.replace(".pt", "")
...
if blend_blurry:
    ...
    print("no weighted average applied (pure evaluation)")
...
# fig.suptitle(model_name_plus_suffix, fontsize=10, y=1.02)
fig.suptitle(f"{model_name_plus_suffix}  |  caption: {caption_source_tag}", fontsize=10, y=1.02)
plt.tight_layout()

os.makedirs('figures/', exist_ok=True)
save_path = f'figures/{model_name_plus_suffix}_sample_grid.png'
...
brain_region_masks = {}
with h5py.File("datasets/brain_region_masks.hdf5", "r") as file:
...
# DOPO
print(f"[ABSOLUTE] METEOR {caption_source_tag} from brain vs captions: {meteor_brain_ref['meteor']}")
...
with torch.no_grad():
    input_ids=tokenizer(list(all_predcaptions),return_tensors="pt",padding=True,truncation=True,max_length=77)
    embedding_brain= model_clip.get_text_features(**input_ids)

    input_ids=tokenizer(list(all_captions),return_tensors="pt",padding=True,truncation=True,max_length=77)
    embedding_captions= model_clip.get_text_features(**input_ids)

    input_ids=tokenizer(all_git_generated_captions,return_tensors="pt",padding=True,truncation=True,max_length=77)
    embedding_images= model_clip.get_text_features(**input_ids)
...
# df.reset_index().to_csv(f'tables/{model_name_plus_suffix}_caption_metrics.csv', sep=',', index=False)
...
# all_prior_out = torch.load(f"evals/{model_name}/{model_name}_all_prior_outs.pt")
# all_clipvoxels = torch.load(f"evals/{model_name}/{model_name}_all_clipvoxels.pt")
